...
namespace Service
{
    internal class DBWork
    {
        ...
        static public bool MakeDB(string _dbname = "AutoService.db")
        {
            ...
            string init_data_mechanic = "INSERT INTO Mechanic (number, name, coefficient) " +
                        "VALUES " +
                        "(1, 'Иванов', 1)," +
                        "(2, 'Петров', 1)," +
                        "(3, 'Сидоров', 0.75)," +
            ...
            string init_data_jobs = "INSERT INTO Jobs " + " (number,name,standartHours,cost," +
                        " client, Mechanic_id) " +
                        "VALUES " +
                        " (1,'Прокачака тормозной системы', 1.5, 3000, 'х001хх',1)," +
                        " (2,'Замена масла', 1, 3000, 'х002хх',3)," +
                        " (3,'Консультация', 0.5, 3000, 'х003хх',2)," +
                        " (4,'Замена лампочки', 0.7, 3000, 'х004хх',4);";
        ...
        static public string GetCoefficient(string _name) // Получаем кооэф. из БД по имени мастера
        {
            string result = string.Empty;
            string get_coefficient = $"SELECT coefficient FROM Mechanic WHERE Name = '{_name}'";
            using (SQLiteConnection conn = new SQLiteConnection(path))
            {
                SQLiteCommand cmd = conn.CreateCommand();
                cmd.CommandText = get_coefficient;
                conn.Open();
                var reader = cmd.ExecuteReader();
                if (reader.HasRows)
                    while (reader.Read())
        ...
        static public void AddData(string _newCategoryInsert, string _dbname = "AutoService.db")
        {
            string path = $"Data Source={_dbname};";
        ...
        static public DataSet Refresh(string _dbname = "AutoService.db")
        {
        ...
        static public void Save(DataTable dt, out string _query, string _dbname = "AutoService.db")
        {
